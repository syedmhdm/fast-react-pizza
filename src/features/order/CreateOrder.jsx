...
function CreateOrder() {
  ...
  const totalPirce = totalCartPirce + priorityPrice;
      ...
      <Form method="POST" action="/fast-react-pizza/order/new">
        ...
        <div>
          ...
          <Button type="primary" disabled={isSubmitting || isLoadingAddress}>
            {isSubmitting
              ? 'Placing Order....'
              : ` Order now from ${formatCurrency(totalPirce)}`}
          </Button>
        </div>
      </Form>
    </div>
...
export async function action({ request }) {
  ...
  // if every thin is okay create new order and redirect
  const newOrder = await createOrder(order);

  // do not over use (it deactivates optimizations)
